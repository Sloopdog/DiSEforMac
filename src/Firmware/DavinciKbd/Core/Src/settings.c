...
void InitSettings()
{
	...
	else if (savedsize == sizeof(sLegacyGeneralSettings))
	{
		sLegacyGeneralSettings legacy = {0};
		FlashRead(&legacy, savedsize);
		memset(&gSettings, 0, sizeof(gSettings));
		memcpy(&gSettings, &legacy, sizeof(legacy));
	}
	else

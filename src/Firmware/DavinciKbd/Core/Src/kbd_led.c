...
static void SendKeyLabel(uint8_t keyid);
...
static void PollHidComm(void)
{
	if (!USBD_HID_Busy(&hUsbDeviceFS, CUSTOM_HID_EPIN_ADDR))
	{
			if (getKeyRequest > 0) {
				...
				getKeyRequest = 0;
			} else if (getKeyLabelRequest > 0) {
				SendKeyLabel(getKeyLabelRequest);
				getKeyLabelRequest = 0;
...
static void SendKeyLabel(uint8_t keyid)
{
	if (keyid > MAX_NUM_KEYS)
		return;
	sMsgKeyLabel *msg = (sMsgKeyLabel *)hid_report_buff;
	msg->cmd = MSG_KEY_LABEL;
	msg->datalen = 1 + KEY_LABEL_LEN;
	msg->keyid = keyid;
	memcpy(msg->label, gSettings.keyLabels[keyid - 1], KEY_LABEL_LEN);
	SendHidMessage();
}
...
static void UpdateKeyLabel(void)
{
	sMsgKeyLabel *msg = (sMsgKeyLabel *)pending_msg;
	int keyid = msg->keyid - 1;
	if (keyid >= 0 && keyid < MAX_NUM_KEYS)
	{
		memcpy(gSettings.keyLabels[keyid], msg->label, KEY_LABEL_LEN);
		gSettings.keyLabels[keyid][KEY_LABEL_LEN - 1] = 0;
	}
}
...
static void GetKeyLabel(void)
{
	sMsgGetKey *msg = (sMsgGetKey *)pending_msg;
	getKeyLabelRequest = msg->keyid;
}
...
static void HandleMessage(void)
{
	...
	case MSG_SET_KEY_LABEL:    UpdateKeyLabel(); break;
	case MSG_GET_KEY_LABEL:    GetKeyLabel(); break;
	}
